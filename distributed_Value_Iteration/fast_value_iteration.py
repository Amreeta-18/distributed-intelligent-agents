...
def evaluate_policy_discounted(env, policy, discount_factor, trials = 1000):
    ...
    for _ in range(trials):
        ...
        observation, reward, done, info = env.step(policy[0])
        total_reward += (discount_factor ** step_index) * reward
        while not done:
            step_index += 1
            observation, reward, done, info = env.step(policy[observation])
            total_reward += (discount_factor ** step_index) * reward
...
def VI_worker_v2(VI_server, data, start_state, end_state):
        ...
        #create container for updated values
        v_new = [0] * (end_state - start_state)
        pi_new = [0] * (end_state - start_state)
        ...
        for update_state in range(start_state, end_state):
            ...
            for action in range(A):
                ...
                tmp_sum = 0
                for next_state, transition_prob in successors:
                    tmp_sum += transition_prob * V[next_state]

                tmp_v = env.GetReward(update_state, action) + (beta * tmp_sum)
                if tmp_v > max_v:
                    max_v = tmp_v
                    max_a = action
            v_new[update_state - start_state] = max_v
            pi_new[update_state - start_state] = max_a

        VI_server.update.remote(start_state, end_state, v_new, pi_new)
...
def fast_value_iteration(env, beta = 0.999, epsilon = 0.01, workers_num = 4):
    ...
    while error > epsilon:
        #INSERT YOUR CODE HERE
        for start_state in range(0, S-1, increment):
            # Wait for all workers finishing, get its reuslt, and delete it from list
            ...
            end_state = start_state + increment
            #print("start_state=%d end_state=%d" % (start_state, end_state))
            w_id = VI_worker_v2.remote(VI_server, data_id, start_state, end_state)
            workers_list.append(w_id)
        #start = 0
        error = ray.get(VI_server.get_error_and_update.remote())
    #wait for all non-idle workers to finish
    ray.wait(workers_list, num_returns = workers_num, timeout = None)
