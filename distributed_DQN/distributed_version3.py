    ...
    def learn(self, test_interval, memory, server):
        for episode in tqdm(range(test_interval), desc="Training"):
            ...
            while steps < self.max_episode_steps and not done:
                ...
                #print("------------------------------MEMORY_SIZE: ", memory.get_current_size())
...
class distributed_DQN_agent():
    ...
    def learn_and_evaluate(self, training_episodes, test_interval):
        ...
        for i in range(test_number):
            ...
            #initiate all collectors
            for j in range(self.cw_num):
                w_id = collectors[j].learn.remote(test_interval, self.memory, self.server)
            ...
            #initiate all evaluators
            for j in range(self.ew_num):
                w_id = evaluators[j].evaluate.remote(self.server)
                workers_id.append(w_id)
...
#debug
training_episodes, test_interval = 1000, 50
num_collectors, num_evaluators = 8, 4
